...
def ler(dicionario, chave):
	if chave in dicionario:
		valor = dicionario[chave]
		return valor
	return '[]'

def escrever(dicionario, chave, valor):
	if chave in dicionario:
...
def atendeRequisicoes(clisock, endr, dicionario):
	while True:
		...
		if not data: #dados vazios: cliente encerrou
			...
			lock.acquire()
			saida = ler(dicionario, str(chave, encoding='utf-8'))
			lock.release()
			...
			lock.acquire()
			escrever(dicionario, str(chave, encoding='utf-8'), str(valor, encoding='utf-8'))
			lock.release()
			print(dicionario)
